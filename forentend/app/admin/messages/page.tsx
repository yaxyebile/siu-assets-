...
import { getUsers, type User } from "@/services/api-service"
...
const MESSAGE_TEMPLATES = [
  ...
  { label: "Digniin", body: "Digniin: Waxaa la ogaaday in shaqooyinka qaar aan si sax ah u dhicin. Fadlan hagaaji." },
  ...
  { label: "Fasax ogaysiis", body: "Fadlan ogaysiis ka bixin fasax, si aad u hesho ogolaansho." },
]
...
export default function MessagesPage() {
  ...
  const [sendMode, setSendMode] = useState<"single" | "bulk" | "user">("single")
  ...
  const handleSendSingle = async () => {
    ...
    const result = await sendSMS(phone, message, phone)
    const updated = [result, ...sentMessages]
    saveMessages(updated)

    if (result.status === "sent") {
      setFeedback({ type: "success", message: `SMS waa loo diray ${phone}` })
      setPhone("")
      setMessage("")
    } else {
  ...
  const handleSendToUser = async () => {
    ...
    if (result.status === "sent") {
      ...
      setFeedback({ type: "error", message: "SMS ma dirin karin." })
    }
    setIsSending(false)
  }

  const handleSendBulk = async () => {
    const validNumbers = bulkNumbers.filter((n) => n.trim())
    if (validNumbers.length === 0 || !message) return
    setIsSending(true)
    setFeedback(null)

    const results: SentMessage[] = []
    for (const num of validNumbers) {
      const result = await sendSMS(num.trim(), message, num.trim())
      results.push(result)
    }
    ...
    if (failCount === 0) {
      setFeedback({ type: "success", message: `${successCount} SMS oo dhan waa la diray!` })
    } else {
  ...
  const handleSend = () => {
    if (sendMode === "single") handleSendSingle()
    else if (sendMode === "user") handleSendToUser()
    else handleSendBulk()
  }

  const addBulkNumber = () => setBulkNumbers([...bulkNumbers, ""])
  const removeBulkNumber = (index: number) => {
    const updated = bulkNumbers.filter((_, i) => i !== index)
    setBulkNumbers(updated.length === 0 ? [""] : updated)
  }
  const updateBulkNumber = (index: number, value: string) => {
  ...
  return (
    <DashboardLayout allowedRoles={["admin"]}>
      <PageHeader title="SMS Messages" description="U dir SMS fariimaha shaqaalaha iyo qolada kale" />
        ...
        <Card className="border-border">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">U dir SMS qof ama koox</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant={sendMode === "user" ? "default" : "outline"}
                size="sm"
                className="gap-2"
                ...
                >
                  ...
                  {users.map((u) => (
                    <option key={u.id} value={u.id}>
                      {u.name} ({u.role === "adminOfficer" ? "Officer" : "Operation"}) {u.phone ? `- ${u.phone}` : "- No Phone"}
                    </option>
                  ))}
                </select>
              </div>
            ...
            {sendMode === "bulk" && (
              <div className="space-y-3">
                ...
                {bulkNumbers.map((num, index) => (
                  <div key={index} className="flex gap-2">
                    <Input
                      type="tel"
                      placeholder={`Number ${index + 1}`}
                      value={num}
                      onChange={(e) => updateBulkNumber(index, e.target.value)}
                      className="bg-muted/50"
                    />
                    {bulkNumbers.length > 1 && (
                      <Button
                        ...
                        onClick={() => removeBulkNumber(index)}
                      >
            ...
            <Button
              ...
              onClick={handleSend}
              disabled={isSending || !message || (sendMode === "single" && !phone) || (sendMode === "user" && !selectedUser)}
            >
              {isSending ? (
                <>
